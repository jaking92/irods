    ...
    auto create_new_replica(rsComm_t& _comm, dataObjInp_t& _inp, DataObjInfo* _existing_replica_list) -> int
    {
        ...
        try {
            ...
            if (irods::CFG_SERVICE_ROLE_PROVIDER != svc_role) {
                ...
                // a catalog provider, this information will not show up in the L1 descriptor
                // because there is no open L1 descriptor on the catalog provider. In this case,
                // we need to extract the relevant information from condInput keywords which
                // contain the information generated by the catalog.
                registered_replica.data_id(std::stoll(l1_cond_input.at(DATA_ID_KW).value().data()));
                registered_replica.owner_user_name(l1_cond_input.at(DATA_OWNER_KW).value());
                registered_replica.owner_zone_name(l1_cond_input.at(DATA_OWNER_ZONE_KW).value());
                registered_replica.ctime(l1_cond_input.at(DATA_CREATE_KW).value());
                registered_replica.mtime(l1_cond_input.at(DATA_MODIFY_KW).value());
                registered_replica.data_expiry(l1_cond_input.at(DATA_EXPIRY_KW).value());
            }
