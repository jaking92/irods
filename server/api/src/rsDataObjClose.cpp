    ...
    const int l1descInx)
{
    ...
    const rodsLong_t vault_size = getSizeInVault(rsComm, L1desc[l1descInx].dataObjInfo);
    if (vault_size < 0) {
        rodsLog( LOG_ERROR,
    ...
    //if ( L1desc[l1descInx].dataObjInfo->dataSize != vault_size ) {
        auto p = ix::make_key_value_proxy(L1desc[l1descInx].dataObjInp->condInput);
        ...
        const int status = _modDataObjSize( rsComm, l1descInx, L1desc[l1descInx].dataObjInfo );
        if ( status < 0 ) {
            rodsLog( LOG_ERROR,
    ...
    const int l1descInx)
{
    ...
    if (l3descInx < 3) {
        THROW(l3descInx, "invalid l3 descriptor index");
    }

    if (const int status = l3Close(comm, l1descInx); status < 0) {
        THROW(status,
            fmt::format("{}: l3Close of {} failed, status = {}",
    ...
    openedDataObjInp_t *dataObjCloseInp)
{
    ...
    if (L1desc[l1descInx].remoteZoneHost) {
        dataObjCloseInp->l1descInx = L1desc[l1descInx].remoteL1descInx;
        int status = rcDataObjClose( L1desc[l1descInx].remoteZoneHost->conn, dataObjCloseInp );
    ...
    try {
        ...
        status = finalize_replica(rsComm, l1descInx, *dataObjCloseInp);
        ...
        apply_static_peps(rsComm, dataObjCloseInp, l1descInx, status);
    }
    catch (const irods::exception& e) {
        irods::log(e);
        return e.code();
