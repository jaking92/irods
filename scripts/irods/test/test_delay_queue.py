...
class Test_Delay_Queue(session.make_sessions_mixin([('otherrods', 'rods')], [('alice', 'apass')]), unittest.TestCase):
    ...
    def metadata_attr_with_value_exists(self, attr, value):
        print('looking for attr:[{0}] value:[{1}]'.format(attr, value))
        out, _, _ = self.admin.run_icommand(['iquest', '%s',
    ...
    def test_failed_delay_job(self):
        ...
        try:
            ...
            with lib.file_backed_up(server_config_filename):
                ...
                lib.delayAssert(
                    lambda: self.metadata_attr_with_value_exists(
                        parameters['metadata_attr'],
                        'We are about to fail...')
                )
                self.assertFalse(self.metadata_attr_with_value_exists(
                    parameters['metadata_attr'],
