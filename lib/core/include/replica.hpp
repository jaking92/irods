...
namespace irods::experimental::replica
{
    ...
    namespace
    {
            ...
            const replica_number_type _rn) -> std::vector<std::string>
        {
            ...
            const std::string qstr = fmt::format(
                ...
                _p.object_name().string(), _p.parent_path().string(), _rn);
        ...
        auto replica_mtime(
            rxComm& _comm,
            const fs::path& _p,
            const replica_number_type _rn) -> std::string
        {
    ...
    /// \since 4.2.9
    template<typename rxComm>
    auto replica_size(rxComm& _comm, const fs::path& _p, const replica_number_type _rn) -> std::uintmax_t
    {
    ...
    /// \since 4.2.9
    template<typename rxComm>
    auto is_replica_empty(rxComm& _comm, const fs::path& _p, const replica_number_type _rn) -> bool
    {
    ...
    /// \since 4.2.9
    template<typename rxComm>
    auto last_write_time(rxComm& _comm, const fs::path& _p, const replica_number_type _rn) -> fs::object_time_type
    {
        detail::throw_if_replica_input_is_invalid(_comm, _p, _rn);

        return fs::object_time_type{std::chrono::seconds{std::stoull(replica_mtime(_comm, _p, _rn))}};
    } // last_write_time
